...
void AudioDevicesImpl::CheckAndSetNewEffectiveDevice(FString newEffectiveId)
{
    // Due to a rare inherent race condition possible when handling simultaneous or back-to-back hotswap events, or when calling SetActiveDevice()
    // immediately prior to device removal, we should quit early if the new effective device ID is both (1) not empty or "No Device" and (2) can
    // no longer be found in _availableDevices, trusting that _effectiveDeviceId will be (or has already been) correctly set by other handlers.

    // Start with the "Check" part...
    IAudioDevice **newEffectiveDevice = nullptr;
    bool isEmptyOrNullDevice = (newEffectiveId.IsEmpty() || newEffectiveId == NULL_DEVICE_ID);
    if (isEmptyOrNullDevice || (newEffectiveDevice = _availableDevices.Find(newEffectiveId)) != nullptr)
    {
        // Either not a physical device, or physical device is available, so...
        // Move on to the "Set" part...
        if (MaybeChangeDeviceId(_effectiveDeviceId, newEffectiveId))
        {
            if (_effectiveDeviceId.IsEmpty())
            {
                ...
                EventEffectiveDeviceChanged.Broadcast(**newEffectiveDevice);
            }
        }
    }
    else
    {
        UE_LOG(VivoxCore, Warning, TEXT("An internal guard prevented the effective audio device from being set to a physical device no longer connected to the system (\"%s\"). "
                                        "Don't worry: this should be automatically corrected for in subsequent internal handling, and the true value shortly assigned.")
                                        , *newEffectiveId);
    }
}
...
VivoxCoreError AudioDevicesImpl::SetActiveDevice(const IAudioDevice& device, const FOnSetActiveDeviceCompletedDelegate& theDelegate)
{
    ...
    innerDelegate.BindLambda([this, theDelegate](const vx_resp_base_t &resp)
    {
        ...
        if(resp.return_code == 0)
        {
            ...
            CheckAndSetNewEffectiveDevice(physicalId);
        }
...
void AudioDevicesImpl::Handle(const vx_resp_aux_get_capture_devices_t &r)
{
    ...
    CheckAndSetNewEffectiveDevice(r.effective_capture_device->device);
...
void AudioDevicesImpl::Handle(const vx_resp_aux_get_render_devices_t &r)
{
    ...
    CheckAndSetNewEffectiveDevice(r.effective_render_device->device);
